...
#include "argos_bridge/StateList.h"
...
void CArgosRosBot::Init(TConfigurationNode& t_node) {
  ...
  stringstream puckListTopic, proximityTopic, stateListTopic;

  puckListTopic << "/" << GetId() << "/puck_list";
  proximityTopic << "/" << GetId() << "/proximity";
  stateListTopic << "/" << GetId() << "/state_list";
  ...
  statePub = nodeHandle->advertise<StateList>(stateListTopic.str(), 1);

  // Create the subscribers
  stringstream cmdVelTopic, hapticTopic;
  ...
  hapticSub = nodeHandle->subscribe(hapticTopic.str(), 1, &CArgosRosBot::hapticCallback, this);
...
void CArgosRosBot::ControlStep() {
  ...
  // StateList stateList;
  // stateList.n = camReads.BlobList.size();
  // for (size_t i = 0; i < stateList.n; ++i) {
  //   State state;
  //
  //   stateList.states.push_back(state);
  // }
  //
  // stateListPub.publish(stateList);
  ...
  // If we haven't heard from the subscriber in a while, set the forcese to zero.
