...
class Game:
    ...
    def get_input_haptic(self, js):
        if not rospy.is_shutdown():
            self.ee_state[0] = self.remap(js.position[0], -100, 100, -pi, pi)
            self.ee_state[1] = self.remap(js.position[1], -100, 150, 0, 20)
            self.ee_state[2] = self.remap(js.position[2], -50, 50, 0, 10)
